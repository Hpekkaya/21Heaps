class MinHeap:
    ...
    def _sink_down(self, index):
        # 1 Initialize the min index
        min_index= index
        # 2 Iterate thorough the heap
        while True:
            # 21 Initialize left and right index
            left_index= self._left_child(index)
            right_index = self._right_child(index)

            # 22 check left and right values with min value then update min index
            ...
            # 24 check if not = min then swap and set index to min_index
            # else break the loop
